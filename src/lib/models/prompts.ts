...
- **두 AI 중 한 곳이라도 숫자를 제시했다면 반드시 종합 숫자를 도출**하세요:
  * 둘 다 숫자 제시 → 두 값의 **중앙값 또는 평균**, 또는 본인 판단으로 조정
  * 한 쪽만 숫자 제시 → 그 값을 그대로 쓰거나 약간 보정
  * 둘 다 null + 합의가 관망 → 그때만 null 허용, 단 entry_zone은 반드시 명시
- 관망 합의여도 "이 가격까지 가면 익절", "이 가격 깨지면 손절"의 **트리거 가격은 제시 가능하고 사용자에게 매우 유용**합니다.
- entry_zone은 항상 권장: "어느 구간에서 재진입/관망 해제 고려하는지" 자유 텍스트로.
