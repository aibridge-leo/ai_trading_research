...
GPT, Gemini, Claude 세 AI가 동일 종목에 대해 3 라운드 토론을 마쳤습니다.

임무:
1. 실시간 웹 검색을 적극 사용해 토론에서 빠진 최신 정보(오늘자 뉴스, 가격 변동, 실적 일정 등)를 보완
2. 세 AI의 최종(3차) 의견을 정리하여 합의/대립 지점을 파악
...
export function buildPerplexityPrompt(args: {
  ticker: string;
  quote: QuoteSnapshot | null;
  finalOpinions: ModelOpinion[]; // 3차 의견 3개
}) {
...
세 AI의 최종 의견을 토대로, 실시간 웹 검색으로 최신 정보를 보강하여 사용자에게 전달할 **종합 분석**을 작성하세요.
