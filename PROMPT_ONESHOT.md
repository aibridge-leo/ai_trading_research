...
미국 주식 티커를 입력하면 **GPT · Gemini** 2개 LLM이 실시간 웹서치 기반으로 **3 라운드 토론**(독립 의견 → 상호 검토 → 최종 입장)을 진행하고, **Perplexity**가 6개 의견을 받아 실시간 서치로 보강한 **종합 분석**을 내는 대시보드를 만들어 주세요. 모든 응답은 한국어 강제, 다크 모드 트레이딩 대시보드 톤.
...
| AI SDK | Vercel `ai` v6 + `@ai-sdk/openai` v3 + `@ai-sdk/google` v3 |
...
> **주의**: AI SDK v5/v6에서 모델 어댑터의 web search tool API가 v4와 다릅니다. 반드시 `@ai-sdk/google@^3`, `@ai-sdk/openai@^3`, `ai@^6` 사용. v1.x를 쓰면 Gemini의 web search 툴이 노출되지 않습니다.
...
각 파일에 **저비용 대체 모델**을 주석으로 명시: `gpt-5.4-mini` / `gemini-3-flash-preview`.
...
Round 1: GPT, Gemini 병렬 호출 (각자 독립 의견)
   │
   ▼
Round 2: 각 모델에 "다른 모델의 1차 의견"을 컨텍스트로 주입 → 반박/동의/보완
   │
   ▼
Round 3: 각 모델에 "다른 모델의 2차 의견" 주입 → 최종 입장
   │
   ▼
Perplexity sonar-reasoning-pro: 6개 의견 + 실시간 서치 → 종합 요약
   │
   ▼
JSON 파일로 이력 저장
```

- **라운드 간 직렬, 라운드 내 2개 모델 병렬** (`Promise.all`)
- 모델 호출 실패 시 placeholder opinion으로 graceful degrade (다른 모델 진행 계속)
- **모델 고정**: GPT + Gemini 2개 모델 고정. 토글 UI 없음.
...
분석 완료 시 `data/history/{ISO타임스탬프}_{TICKER}.json`에 6개 의견 + 종합 전체 보존. `data/history/.gitkeep` 추가하고 `.gitignore`에 `/data/history/*.json !.gitkeep`.
...
2. **AI SDK 버전** → v1.x는 Gemini의 web search tool이 노출 안 됨. 반드시 v6+ (`@ai-sdk/google@^3` 등).
...
2. `.env.local`에 3개 API 키 입력 (OpenAI / Google / Perplexity)
